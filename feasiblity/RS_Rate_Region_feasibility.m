...
% % % test only
% % Rth = 0; 
% 0, should be all feasible, 100 low SNR should not be feasible
% % Rth = 0,0.5, 1, 10, 100; 
...
for method = 1:4 % loop through 4 different initializaiton methods
    ...
    % trace(p1'*p1)+trace(p2'*p2)+trace(pc'*pc)
    loop=1;
    AWSMSE_old=0.1;
    count=0;
    while (loop)
        ...
        %Step 4: Update P
        [feasible(method), AWSMSE,p1,p2,pc]=RS_update_P_feasibility(Rth, H_est,weights,SNR,Uc1,Uc2,U1,U2,tc1,tc2,t1,t2,...
